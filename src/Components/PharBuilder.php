...
private $directories = [];
private $supportedSignatureTypes = ['SHA-512' => \Phar::SHA512, 'SHA-256' => \Phar::SHA256, 'SHA-1' => \Phar::SHA1];
/**
 * @var array
 */
private $options = [
...
public function setSignatureType($type)
{
    if (!array_key_exists($type, $this->supportedSignatureTypes)) {
        throw new \InvalidArgumentException(sprintf('Signature type "%s" not known or not supported by this PHP installation.', $type));
    }
...
* @return callable
*/
public function getIteratorFilter()
{
